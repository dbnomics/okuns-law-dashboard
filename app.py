...
def main():
    st.set_page_config(
        ...
        page_icon="DB_logo")
    st.image("dbnomics_2.svg", width=300)
    ...
    with tab2:
        ...
        selected_countries = st.multiselect('**Select Country**', countries, default=['France'])

        data = load_data()

        if selected_countries:
            all_dates = pd.to_datetime(data[selected_countries[0]]['period']).sort_values().unique()
        ...
        st.write(":blue[Unemploymant rate]: [*link*](https://db.nomics.world/ILO/UNE_DEAP_SEX_AGE_EDU_RT?tab=list)")
        st.write(":blue[GDP growth rate]: [*link*](https://db.nomics.world/IMF/WEO:2024-04?tab=list)")
